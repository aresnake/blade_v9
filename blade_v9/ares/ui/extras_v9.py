...
class BLADEV9_OT_export_zip(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        with # [V9] disabled packaging: \1zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
            for root, _, files in os.walk(addon_root):
                for f in files:
                    full = os.path.join(root, f)
                    rel  = os.path.relpath(full, start=os.path.dirname(addon_root))  # -> blade_v9/...
                    zf.write(full, rel)
        self.report({"INFO"}, f"# [V9] disabled log: Exported: {zip_path}")
        return {"FINISHED"}
...
class BLADEV9_OT_reload_scripts(bpy.types.Operator):
    ...
    def execute(self, context):
        try:
            # # bpy.ops.script.reload()  # disabled dev  # disabled GUI
